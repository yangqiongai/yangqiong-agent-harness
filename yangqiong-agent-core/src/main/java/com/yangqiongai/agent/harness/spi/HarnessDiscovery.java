...
public final class HarnessDiscovery {
    ...
    public static DiscoveredExtensions discover(ClassLoader classLoader) {
        ...
        List<com.yangqiongai.agent.harness.core.middleware.AgentMiddleware> middlewares =
                new ArrayList<>();
        List<com.yangqiongai.agent.harness.core.tool.AgentTool> tools = new ArrayList<>();
        List<com.yangqiongai.agent.harness.core.model.AgentModel> models = new ArrayList<>();
        List<com.yangqiongai.agent.harness.durable.DistributedStores> stores = new ArrayList<>();
        List<com.yangqiongai.agent.harness.guardrail.InjectionDetector> detectors = new ArrayList<>();
        ...
        storeProviders.stream().sorted(Comparator.comparingInt(StoreProvider::order))
                .forEach(p -> {
                    com.yangqiongai.agent.harness.durable.DistributedStores s = p.provideStores();
                    if (s != null) {
                        stores.add(s);
                    }
                });
