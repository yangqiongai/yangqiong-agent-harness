 ...
 * 落盘向量存储
 * <p>
 * 通用持久的向量后端，不绑定具体实现：基于同一VectorIndex同时提供
 * 长期向量记忆（PersistentVectorMemory）与语义向量检索（VectorRetriever），
 * 嵌入模型由调用方注入（可复用以OllamaEmbeddingModel为代表的任意EmbeddingModel实现）。
 * 支持两种打开方式：本地Lucene数据目录（open(Path, EmbeddingModel)），
 * 或外部向量索引（open(VectorIndex, EmbeddingModel)，适配pgvector、Milvus、Qdrant等独立向量服务）。
 * </p>
 * @author yangqiong
 */
public final class PersistentVectorStore implements AutoCloseable {
    ...
    private PersistentVectorStore(VectorIndex index, EmbeddingModel embeddingModel) {
    ...
    public static PersistentVectorStore open(Path dataDir, EmbeddingModel embeddingModel) {
        ...
        return open(new LuceneVectorIndex(dataDir), embeddingModel);
    }

    /**
     * 以外部向量索引打开向量存储，适配独立向量服务后端
     * @param index
     * @param embeddingModel
     * @return
     */
    public static PersistentVectorStore open(VectorIndex index, EmbeddingModel embeddingModel) {
        return new PersistentVectorStore(index, embeddingModel);
    }
