...
class ParadigmClarificationTest {
    ...
    private void stubAskUserTool() {
        when(toolExecutor.executeTools(any(), any(), any())).thenAnswer(invocation -> {
            List<com.yangqiongai.agent.harness.core.message.AgentToolUseBlock> calls = invocation.getArgument(0);
            List<AgentMessage> results = new ArrayList<>();
            for (com.yangqiongai.agent.harness.core.message.AgentToolUseBlock call : calls) {
