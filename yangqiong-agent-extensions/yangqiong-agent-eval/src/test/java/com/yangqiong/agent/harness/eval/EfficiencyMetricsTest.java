...
class EfficiencyMetricsTest {
    ...
    static class MultiRoundRuntime implements AgentRuntime {
        ...
        @Override
        public Mono<com.yangqiong.agent.harness.core.message.AgentMessage> call(
                List<com.yangqiong.agent.harness.core.message.AgentMessage> inputs,
                AgentRuntimeContext context) {
            return Mono.just(MessageFactory.createUserMessage("桩运行时同步结果"));
        }

        @Override
        public Flux<AgentEvent> stream(List<com.yangqiong.agent.harness.core.message.AgentMessage> inputs,
                                       AgentRuntimeContext context) {
            return Flux.just(
